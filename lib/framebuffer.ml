...
let pp_backend_event (fmt : Format.formatter) : S.backend_event -> unit =
  function
  | Clipboard_paste str ->
    Fmt.pf fmt "Clipboard_paste: %S" str
  | Clipboard_request ->
    Fmt.pf fmt "Clipboard_request"
  | Keypress {pressed; scancode; mask; mods; keysym} ->
    Fmt.pf fmt
      "Keypress {pressed: %b scancode: %d mask: %d; mods: [%a] ; keysym: %a}"
      pressed scancode mask
      Fmt.(list ~sep:(unit "; ") Keycodes.pp_kmod)
      mods
      Fmt.(option Keycodes.pp_keysym)
      keysym
  | Mouse_button {x; y} ->
    Fmt.pf fmt "Mouse_button (x: %d, y: %d)" x y
  | Mouse_motion {x; y} ->
    Fmt.pf fmt "Mouse_motion (x: %d, y: %d)" x y
  | Window_close ->
    Fmt.pf fmt "Window_close"
  | Window_focus ->
    Fmt.pf fmt "Window_focus"
  | Resize (w, h) ->
    Fmt.pf fmt "Resize: %d x %d" w h

module Make : S.Framebuffer_M =
functor
  (Backend : S.Backend_S)
  ->
  struct
    let init init_t =
      Backend.init_backend init_t
      >>= fun () ->
      let module Frontend :
        S.Framebuffer_S
        with type color = Backend.color
         and type line = Backend.line = struct
        ...
        module Log : Logs.LOG =
        ( val Logs.src_log
                (Logs.Src.create "framebuffer" ~doc:"Mirage.Framebuffer") )
        ...
        let compile_font backend : font_table =
          ...
          let () =
            let str =
              match Bitmapfont.read "single-chars.bitmap" with
              | Some s ->
                s
              | _ ->
                failwith "unable to load bitmap content"
            in
            let pixels_of_byte byte =
              backend
              |> ( font_w
                 |> Backend.Compile.lineiter (fun i ->
                        if byte land (1 lsl (font_w - i - 1)) > 0 then color
                        else blank ) )
            in
            for chari = 0 to (String.(length str) - 1) / codepoint_w do
              let t = Cstruct.create codepoint_w in
              let () =
                Cstruct.blit_from_string str (chari * codepoint_w) t 0
                  codepoint_w
              in
              Hashtbl.add font
                ...
                Array.(
                  init font_h (fun i ->
                      pixels_of_byte @@ Cstruct.(get_uint8 t (2 + i)) ))
              (* 2 = codepoint uint16 *)
            done
          in
          font
        ...
        let letter (t : t) codepoint ~x ~y =
          ...
          assert (x <= t.height);
          let letter =
            try Hashtbl.(find t.font codepoint) with _ ->
              failwith ("letter lookup failed for " ^ string_of_int codepoint)
          in
          let y_end = min t.height (y + font_h) in
          rect_lineiter t ~x ~y ~y_end (fun i -> letter.(i))

        let letters t str ~x ~y =
          stringiteri str (fun i ch ->
              let x2 = min (t.width - 8) (x + (8 * i)) in
              let y2 = min y (t.height - 16) in
              (*Log.warn (fun m -> m "i: %d x: (%d) %d y: (%d) %d c: %C" i x2 x
                y2 y ch);*)
        ...
        let window ~width ~height : t Lwt.t =
          Backend.window init_t ~height ~width
          >>= fun backend ->
          ...
          let t : t =
            ...
            ; b = backend
            ; text_lines =
                ( tty_backlog_size - (height / font_h)
                , Array.make tty_backlog_size "" )
            ; font = compile_font backend }
          in
          Lwt.return t
        ...
        let output_tty (t : t) (width, height) str =
          let line_wrap (width : int) str =
            ...
            let rec wrap acc = function
              | [] ->
                acc
              | hd :: tl
                when String.length hd > width ->
                let hdl = String.length hd in
                wrap
                  (String.sub hd (hdl - width) width :: acc)
                  (String.sub hd 0 (hdl - width) :: tl)
              | hd :: tl ->
                wrap (hd :: acc) tl
            in
            wrap [] @@ String.split_on_char '\n' str
          in
          let take_last n lst =
            let rec loop acc i = function
              ...
              | _
                ...
                acc
              | hd :: tl ->
                loop (hd :: acc) (pred i) tl
            in
            loop [] n (List.rev lst)
          in
          let offset, lines = t.text_lines in
          Logs.debug (fun m ->
              m "width: %d height: %d offset: %d" width height offset );
          let append_to_current, line_list =
            let offset_line_len = String.length lines.(offset) in
            match
              ( offset_line_len < width
              , List.rev (line_wrap width str |> take_last height) )
            with
            | ( true
            ...
            | false, tl ->
              ("", tl) (* can NOT append; we're on a new line *)
            | true, first :: tl
              when offset_line_len + String.length first <= width ->
              (first, tl)
            | true, first :: tl ->
              (* first is too long*)
              ...
              let first_tl =
                String.sub first take_len (String.length first - take_len)
              in
              (first_line, first_tl :: tl)
          in
          let () =
            Logs.info (fun m ->
                m "appending to current line: %S\nStrs: %a" append_to_current
                  Fmt.(list ~sep:(unit "~") string)
                  line_list )
          in
          let dim = Array.length lines in
          let lst_dim = List.length line_list in
          let old = max 0 (dim - lst_dim) in
          (* number of old lines to retain *)
          let dropped = dim - old in
          Logs.debug (fun m ->
              m "adim: %d lst_dim %d old %d dropped %d" dim lst_dim old dropped
          );
          Logs.debug (fun m ->
              m "Current array: %a" Fmt.(array ~sep:(unit ";") string) lines );
          for i = dropped to dim - 1 do
            (* 0 -> (10-1)-2 = 8*)
            (* shift previous line *)
            lines.(i - dropped)
          ...
          Logs.debug (fun m ->
              m "Current array shifted: %a"
                Fmt.(array ~sep:(unit ";") string)
                lines );
          ...
          Logs.debug (fun m ->
              m "New array: %a" Fmt.(array ~sep:(unit ";") string) lines );
          (* only draw what fits into (size t): *)
          let offset = dim - height in
          let rpad_spaces s =
            let len = String.length s in
            if len < width then
              String.concat "" [s; String.make (width - len) ' ']
            else s
          in
          for y = height - 1 downto 0 do
            letters t (rpad_spaces lines.(y + offset)) ~x:0 ~y:(y * 16)
            (*TODO 16 = font_h*)
          done;
          t.text_lines <- (new_offset, lines);
          redraw t
      end in
      (* pack the module and return from [Make.init] *)
